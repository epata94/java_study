...
// 값을 입력할 때 인덱스(위치정보)가 필요없다.
    ...
    private static void showMenu() {
        ...
        while (true) {
            ...
            System.out.println("5. 학생 ID 검색");
            System.out.println("6. 학생 이름 검색");
            System.out.println("7. 국어 점수 검색");
...
//        ArrayList의 요소가 없는 조건을 아래 메소드로 알수 있다.
    ...
    // 6. 학생 이름으로 검색하여 정보 출력
    private static void searchStudentByName() {
        Scanner scanner = new Scanner(System.in);
        System.out.print("검색할 학생 이름을 입력하세요: ");
        String name = scanner.nextLine();

        // 입력받은 이름과 동일한 이름을 가진 학생을 찾아서 출력한다.
        boolean isFound = studentMap.values().stream()
    ...
    // 7. 국어 점수 기준 이상 학생 검색하여 정보 출력
    private static void searchStudentByKoreanScore() {
        Scanner scanner = new Scanner(System.in);
        System.out.print("검색할 국어 점수를 입력하세요: ");
        int koreanScore = scanner.nextInt();

        // 국어 점수가 입력받은 점수 이상인 학생을 찾아서 출력한다.
        boolean isFound = studentMap.values().stream()
                .filter(student -> student.getKoreanScore() >= koreanScore)
                .peek(System.out::println)
                .findAny()
                .isPresent();
